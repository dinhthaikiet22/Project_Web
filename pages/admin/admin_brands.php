    ...
    <div class="col-lg-4">
        <div class="admin-card border-0 shadow-sm rounded-4 sticky-top" style="top: 20px;">
            ...
            <form action="modules/admin/admin_brand_action.php" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    <label for="brand_name" class="form-label fw-semibold">Tên hãng xe <span class="text-danger">*</span></label>
                    <input type="text" id="brand_name" name="name" class="form-control bg-light" placeholder="Ví dụ: Giant, Trek..." required>
                </div>

                <div class="mb-3">
                    <label for="brand_origin" class="form-label fw-semibold">Xuất xứ <span class="text-muted fw-normal">(Quốc gia)</span></label>
                    <input type="text" id="brand_origin" name="origin" class="form-control bg-light" placeholder="Ví dụ: Đài Loan, Mỹ...">
                </div>
                
                <div class="mb-3">
                    <label for="brand_desc" class="form-label fw-semibold">Mô tả ngắn</label>
                    <textarea id="brand_desc" name="description" class="form-control bg-light" rows="3" placeholder="Thông tin về hãng xe..."></textarea>
                </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center gap-3">
                                            <?php if (!empty($brand['image_url'])): ?>
                                                <div class="border rounded p-1 bg-white d-flex align-items-center justify-content-center" style="width: 50px; height: 50px;">
                                                    <img src="<?= str_starts_with((string)$brand['image_url'], 'http') ? htmlspecialchars((string)$brand['image_url'], ENT_QUOTES, 'UTF-8') : 'public/uploads/brands/' . rawurlencode((string)$brand['image_url']) ?>" alt="logo" style="max-width: 100%; max-height: 100%; object-fit: contain;">
                                                </div>
                                            <?php else: ?>
                                                <div class="bg-light border rounded d-flex align-items-center justify-content-center" style="width: 50px; height: 50px; color: #FF5722; font-size: 1.25rem;">
                                            ...
                                            <div>
                                                <div class="fw-bold text-dark fs-6 d-flex align-items-center gap-2">
                                                    ...
                                                    <?php if (!empty($brand['origin'])): ?>
                                                        <span class="badge bg-light text-secondary border fw-normal" style="font-size: 0.70rem;"><i class="fa-solid fa-earth-americas me-1"></i><?= htmlspecialchars((string)$brand['origin'], ENT_QUOTES, 'UTF-8') ?></span>
                                                    <?php endif; ?>
                                                </div>
                                                <div class="text-muted small text-truncate" style="max-width: 250px;" title="<?= htmlspecialchars((string)($brand['description'] ?? ''), ENT_QUOTES, 'UTF-8') ?>">
                                    ...
                                    <td class="text-end">
                                        <div class="d-flex justify-content-end gap-2">
                                            <button onclick="editBrand(<?= $brand['id'] ?>, '<?= htmlspecialchars((string)$brand['name'], ENT_QUOTES, 'UTF-8') ?>')" class="btn btn-sm btn-outline-primary rounded-3 text-primary d-flex align-items-center justify-content-center border-primary" style="width: 34px; height: 34px;" title="Sửa tên">
                                                <i class="fa-solid fa-pen"></i>
                                            </button>
                                            <button onclick="deleteBrand(<?= $brand['id'] ?>, <?= $brand['bike_count'] ?>)" class="btn btn-sm btn-outline-danger rounded-3 text-danger d-flex align-items-center justify-content-center border-danger" style="width: 34px; height: 34px;" title="Xóa">
